...
class AdvancedMixin:
    def handle_nautilus_action(self):
        print("Nautilus box state is being processed...")
    ...
    def add_grid_item(self, label, default_color, css_id):
        ...
        if css_id == "KDE_custom":
            ...
            self.plasma_refresh_switch.connect(
                "notify::active", self.on_plasma_refresh_toggled
            )
            self.plasma_handler_id = self.plasma_refresh_switch.connect(
                "notify::active", self.on_plasma_refresh_toggled
            )
            ...
            self.topbar_row = self.create_color_entry(
                "Top Bar Color", "#3584e4", "topbar-color"
            )
            ...
            self.clock_row = self.create_color_entry(
                "Clock Color", "#f9f9f9", "clock-color"
            )
            ...
            self.gnome_refresh_row = Adw.ActionRow(
                title="Auto-Reload Shell Theme", subtitle=status_text
            )
            ...
            self.gnome_handler_id = self.refresh_switch.connect(
                "notify::active", self.on_gnome_refresh_toggled
            )
        ...
        if css_id == "nautilus_custom":
            ...
            naut_row = self.create_color_entry(
                "Main Hex", default_color, f"{css_id}-main"
            )
            ...
            naut_row_sec = self.create_color_entry(
                "Secondary Hex", default_color, f"{css_id}-sec"
            )
        ...
        btn_layout.set_margin_top(10)
        btn_layout.set_margin_bottom(10)
        btn_layout.set_margin_start(10)
        btn_layout.set_margin_end(10)
        ...
        def update_status(*args):
            ...
            status_label.set_label(
                "Custom" if (is_main_active or is_sec_active) else "Default"
            )
        ...
        if getattr(self, f"{css_id}_sec_switch", None):
            self.__dict__[f"{css_id}_sec_switch"].connect(
                "notify::active", update_status
            )
    ...
    def show_reset_instructions(self, widget):
        ...
        def on_response(d, response_id):
            if response_id == "flatseal":
                Gtk.show_uri(
                    self, f"app://com.github.tchx84.Flatseal", Gdk.CURRENT_TIME
                )
